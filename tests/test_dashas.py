...
from astrovedic.vedic import vimshottari, yogini, chara, ashtottari
...
class TestAshtottariDasha(unittest.TestCase):
    ...
    def test_dasha_balance(self):
        """Test Ashtottari Dasha balance calculation"""
        balance = ashtottari.get_dasha_balance(self.chart)
        self.assertIsInstance(balance, float)
        self.assertTrue(0 <= balance <= 21)  # Max period is Venus at 21 years

    def test_current_dasha(self):
        """Test current Ashtottari Dasha calculation"""
        dasha = ashtottari.get_current_dasha(self.chart)
        self.assertIsNotNone(dasha)
        self.assertIn('mahadasha', dasha)

        # Ashtottari Dasha uses a subset of planets
        ashtottari_planets = [const.SUN, const.MOON, const.MARS, const.MERCURY,
                             const.SATURN, const.JUPITER, const.RAHU, const.VENUS]
        self.assertIn(dasha['mahadasha']['planet'], ashtottari_planets)

    def test_mahadasha(self):
        """Test Mahadasha retrieval"""
        mahadasha = ashtottari.get_mahadasha(self.chart)
        self.assertIsNotNone(mahadasha)

        # Ashtottari Dasha uses a subset of planets
        ashtottari_planets = [const.SUN, const.MOON, const.MARS, const.MERCURY,
                             const.SATURN, const.JUPITER, const.RAHU, const.VENUS]
        self.assertIn(mahadasha['planet'], ashtottari_planets)

    def test_antardasha(self):
        """Test Antardasha retrieval"""
        antardasha = ashtottari.get_antardasha(self.chart)
        # Antardasha might be None for a new chart at birth time
        if antardasha:
            # Ashtottari Dasha uses a subset of planets
            ashtottari_planets = [const.SUN, const.MOON, const.MARS, const.MERCURY,
                                 const.SATURN, const.JUPITER, const.RAHU, const.VENUS]
            self.assertIn(antardasha['planet'], ashtottari_planets)
