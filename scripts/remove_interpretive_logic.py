#!/usr/bin/env python3
"""
Script to remove interpretive logic from Kuta modules
"""
...
def remove_description_function(file_path):
    ...
    pattern = r'def generate_\w+_description\([^)]*\):\s*"""[^"]*""".*?(?=\n\n|$)'
    ...
    pattern = r"# Generate the description\s*description = generate_\w+_description\([^)]*\)"
...
def main():
    ...
    Main function
    """
    # Get all Kuta module files
    kuta_dir = 'astrovedic/vedic/compatibility/kuta'
    kuta_files = glob.glob(f"{kuta_dir}/*.py")
    
    # Process each file
    for file_path in kuta_files:
        if os.path.basename(file_path) not in ['__init__.py', 'varna.py', 'yoni.py']:
