...
from flatlib.vedic.nakshatras import (
    ...
    NAKSHATRA_SPAN, NAKSHATRA_LORDS
)
...
def get_pratyantardasha_sequence(mahadasha_planet, antardasha_planet, antardasha_years):
    """
...
def calculate_dasha_periods(birth_date, moon_longitude):
    ...
    if isinstance(birth_date, Datetime):
        birth_dt = datetime.strptime(f"{birth_date.date} {birth_date.time}", "%Y/%m/%d %H:%M")
    else:
        birth_dt = birth_date
    ...
    for mahadasha in mahadasha_sequence:
        ...
        for antardasha in antardasha_sequence:
            ...
            pratyantardasha_sequence = get_pratyantardasha_sequence(planet, ad_planet, ad_years)
            pratyantardashas = []
