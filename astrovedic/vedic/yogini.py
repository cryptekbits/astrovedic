"""
    ...
    This module implements Yogini Dasha calculations for Vedic astrology.
    Yogini Dasha is a 36-year cycle based on the Moon's Nakshatra.
"""
...
YOGINI_PERIODS = {
    const.MOON: 1,    # Mangala
    const.SUN: 2,     # Pingala
    const.JUPITER: 3, # Dhanya
    const.MERCURY: 4, # Bhramari
    const.SATURN: 5,  # Bhadrika
    const.VENUS: 6,   # Ulka
    const.MARS: 7,    # Siddha
    const.RAHU: 8     # Sankata
}

# Total years in Yogini Dasha cycle
TOTAL_YOGINI_YEARS = sum(YOGINI_PERIODS.values())  # 36 years

# Yogini Dasha sequence
YOGINI_SEQUENCE = [
    const.MOON, const.SUN, const.JUPITER, const.MERCURY,
    const.SATURN, const.VENUS, const.MARS, const.RAHU
]

# Nakshatra to Yogini mapping (1-based nakshatra index to Yogini planet)
NAKSHATRA_TO_YOGINI = {
    1: const.MARS,    # Ashwini -> Siddha
    2: const.RAHU,    # Bharani -> Sankata
    3: const.JUPITER, # Krittika -> Dhanya
    4: const.SUN,     # Rohini -> Pingala
    5: const.MOON,    # Mrigashira -> Mangala
    6: const.SATURN,  # Ardra -> Bhadrika
    7: const.MERCURY, # Punarvasu -> Bhramari
    8: const.VENUS,   # Pushya -> Ulka
    9: const.MARS,    # Ashlesha -> Siddha
    10: const.RAHU,   # Magha -> Sankata
    11: const.JUPITER,# Purva Phalguni -> Dhanya
    12: const.SUN,    # Uttara Phalguni -> Pingala
    13: const.MOON,   # Hasta -> Mangala
    14: const.SATURN, # Chitra -> Bhadrika
    15: const.MERCURY,# Swati -> Bhramari
    16: const.VENUS,  # Vishakha -> Ulka
    17: const.MARS,   # Anuradha -> Siddha
    18: const.RAHU,   # Jyeshtha -> Sankata
    19: const.JUPITER,# Mula -> Dhanya
    20: const.SUN,    # Purva Ashadha -> Pingala
    21: const.MOON,   # Uttara Ashadha -> Mangala
    22: const.SATURN, # Shravana -> Bhadrika
    23: const.MERCURY,# Dhanishta -> Bhramari
    24: const.VENUS,  # Shatabhisha -> Ulka
    25: const.MARS,   # Purva Bhadrapada -> Siddha
    26: const.RAHU,   # Uttara Bhadrapada -> Sankata
    27: const.JUPITER # Revati -> Dhanya
}

def calculate_yogini_dasha_balance(moon_longitude: float) -> float:
    """
    Calculate the balance of the current Yogini Dasha at birth

    Args:
        moon_longitude (float): The Moon's longitude in degrees (0-360)

    Returns:
        float: The balance of the current Yogini Dasha in years
    """
    ...
    # Get the Yogini planet for this nakshatra
    yogini_planet = NAKSHATRA_TO_YOGINI[nakshatra_index]
    
    # Calculate position within nakshatra (0-13.33333 degrees)
    pos_in_nakshatra = nakshatra_info['percentage'] / 100
    
    # Calculate the balance of the current Yogini Dasha
    years_of_dasha = YOGINI_PERIODS[yogini_planet]
    balance = years_of_dasha * (1 - pos_in_nakshatra)
    
    return balance

def get_yogini_sequence(moon_longitude: float) -> List[Dict[str, Any]]:
    """
    Get the sequence of Yogini Dashas starting from birth

    Args:
        moon_longitude (float): The Moon's longitude in degrees (0-360)

    Returns:
        list: List of dictionaries with Yogini Dasha information
    """
    # Get nakshatra information
    nakshatra_info = get_nakshatra(moon_longitude)
    nakshatra_index = nakshatra_info['index'] + 1  # Convert to 1-based index
    ...
    # Calculate the balance of the current Yogini Dasha
    balance = calculate_yogini_dasha_balance(moon_longitude)
    
    # Find the starting index in the Yogini sequence
    start_idx = YOGINI_SEQUENCE.index(yogini_planet)
    
    # Create the sequence of Yogini Dashas
    yogini_sequence = []
    
    # Add the current Yogini Dasha with its balance
    yogini_sequence.append({
        'planet': yogini_planet,
        'years': balance
    })
    
    # Add the remaining Yogini Dashas in sequence
    for i in range(1, 8):
        idx = (start_idx + i) % 8
        planet = YOGINI_SEQUENCE[idx]
        yogini_sequence.append({
            'planet': planet,
            'years': YOGINI_PERIODS[planet]
        })
    
    return yogini_sequence

def get_yogini_antardasha_sequence(mahadasha_planet: str, mahadasha_years: float) -> List[Dict[str, Any]]:
    """
    Get the sequence of Yogini Antardashas (sub-periods) for a given Mahadasha
    ...
    """
    # Find the starting index in the Yogini sequence
    start_idx = YOGINI_SEQUENCE.index(mahadasha_planet)
    ...
    for i in range(8):
        ...
        years = (YOGINI_PERIODS[planet] / TOTAL_YOGINI_YEARS) * mahadasha_years
...
def calculate_yogini_dasha_periods(birth_date: datetime, moon_longitude: float) -> Dict[str, Any]:
    """
    Calculate all Yogini Dasha periods from birth
    ...
    # Get the Yogini Dasha sequence
    yogini_sequence = get_yogini_sequence(moon_longitude)
    
    # Calculate start and end dates for each Mahadasha
    current_date = birth_dt
    mahadashas = []
    
    for mahadasha in yogini_sequence:
        ...
        antardasha_sequence = get_yogini_antardasha_sequence(planet, years)
        antardashas = []
...
def get_current_yogini_dasha(dasha_periods: Dict[str, Any], date: Optional[datetime] = None) -> Optional[Dict[str, Any]]:
    """
    Get the current operating Yogini Dasha period
...
def get_dasha_balance(chart: Chart) -> float:
    """
    Get the balance of the first Yogini Dasha (major period) at birth for a chart.

    Args:
        chart (Chart): The chart object containing birth details.

    Returns:
        float: The balance of the first Yogini Dasha in years.
    ...
    return calculate_yogini_dasha_balance(moon.lon)

def get_current_dasha(chart: Chart, date: Optional[Datetime] = None) -> Optional[Dict[str, Any]]:
    """
    Get the current operating Yogini Dasha for a chart at a specific date.
    ...
    all_periods = calculate_yogini_dasha_periods(chart.date.to_datetime(), moon.lon)
    
    # Find the specific dasha for the target date
    current_dasha_info = get_current_yogini_dasha(all_periods, target_date.to_datetime())

    return current_dasha_info

def get_mahadasha(chart: Chart, date: Optional[Datetime] = None) -> Optional[Dict[str, Any]]:
    """
    Get the current Yogini Mahadasha (major period) for a chart at a specific date.
...
def get_antardasha(chart: Chart, date: Optional[Datetime] = None) -> Optional[Dict[str, Any]]:
    """
    Get the current Yogini Antardasha (sub-period) for a chart at a specific date.
